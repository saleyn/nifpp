...
#if NIFPP_HAS_MAPS
template<typename TK, typename TV> int add_to_map(ErlNifEnv* env, TERM &map, const std::pair<TK,TV> &var);

template<typename TK, typename TV, typename... Pairs>
int add_to_map(ErlNifEnv* env, TERM &map, const std::pair<TK,TV> &var, Pairs&&... pairs);
...
//    the weird stuct/function composite.
    ...
    struct array_to_tupler
    {
        ...
        static int go(ErlNifEnv* env, std::tuple<Ts...> &t, const ERL_NIF_TERM* end)
        {
            ...
            return array_to_tupler<I-1>::go(env, t, end) ? get(env, *end, std::get<I-1>(t)) : 0;
        }
    };
...
int get(ErlNifEnv* env, ERL_NIF_TERM term, std::pair<T1, T2>& var)
{
    ...
    return ret && arity == 2 &&
           get(env, array[0], var.first) && get(env, array[1], var.second);
}
...
int add_to_map(ErlNifEnv* env, TERM &map, const std::pair<TK,TV> &var)
{
