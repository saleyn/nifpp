...
// erlang/26.0/erts-14.0/include/erl_nif.h:192:21: warning: comma at end of enumerator list
// [-Wpedantic]
//  192 |     ERL_NIF_UTF8 = 2,
#include <erl_nif.h>
...
#define NIFPP_HAS_MAPS                                                                             \
  ((ERL_NIF_MAJOR_VERSION > 2) || (ERL_NIF_MAJOR_VERSION == 2 && ERL_NIF_MINOR_VERSION >= 6))
...
template <>
struct hash<nifpp::TERM> {
  std::size_t operator()(const nifpp::TERM& k) const { return hash<ERL_NIF_TERM>()(k.v); }
};

template <>
struct hash<nifpp::str_atom> {
  std::size_t operator()(const nifpp::str_atom& k) const { return hash<std::string>()(k); }
};

template <>
struct hash<nifpp::atom> {
  std::size_t operator()(const nifpp::atom& k) const { return hash<ERL_NIF_TERM>()(k); }
};
...
inline binary_string operator""_b(const char* s, size_t sz)
{
  return std::string_view(s, sz);
}

inline binary_string operator""_s(const char* s, size_t sz)
{
  return std::string_view(s, sz);
}
...
#if NIFPP_HAS_MAPS
// template<typename TK, typename TV> bool add_to_map(ErlNifEnv* env, TERM &map, const
// std::pair<TK,TV>& var);
...
template <typename TK, typename TV>
TERM make(ErlNifEnv* env, const std::map<TK, TV>& var, TERM* add_to_map = NULL);

template <typename TK, typename TV>
bool get(ErlNifEnv* env, ERL_NIF_TERM term, std::unordered_map<TK, TV>& var);
template <typename TK, typename TV>
TERM make(ErlNifEnv* env, const std::unordered_map<TK, TV>& var, TERM* map_to_use = NULL);
#endif
...
inline TERM make(ErlNifEnv* env, const binary_string& v)
{
  return make_binary(env, std::string_view{v});
}
...
#define NIFPP_DEFINE_INTEGER_TRAITS(CppType, GetFunc, MakeFunc)                                    \
  template <>                                                                                      \
  struct integer_nif_traits<CppType> {                                                             \
    static constexpr bool is_supported = true;                                                     \
    static bool get_value(ErlNifEnv* env, ERL_NIF_TERM term, CppType* var)                         \
    {                                                                                              \
      return GetFunc(env, term, var);                                                              \
    }                                                                                              \
    static ERL_NIF_TERM make_value(ErlNifEnv* env, CppType var) { return MakeFunc(env, var); }     \
  };
...
template <typename T>
struct is_nif_integer<T, typename std::enable_if<integer_nif_traits<T>::is_supported>::type>
    : std::true_type {};
...
#define NIFPP_DEFINE_SEQUENTIAL_CONTAINER_TRAITS(ContainerType)                                    \
  template <typename T>                                                                            \
  struct container_traits<ContainerType<T>> {                                                      \
    using value_type = T;                                                                          \
    static constexpr bool is_supported = true;                                                     \
...
template <typename T>
struct is_consolidatable_container<T,
                                   typename std::enable_if<container_traits<T>::is_sequential ||
                                                           container_traits<T>::is_set_like>::type>
    : std::true_type {};
...
struct has_reverse_iterators<
    T, typename std::enable_if<container_traits<T>::has_reverse_iterators>::type> : std::true_type {
};
...
inline typename std::enable_if<detail::is_nif_integer<T>::value && detail::is_nif_integer<I>::value,
                               bool>::type
get(ErlNifEnv* env, ERL_NIF_TERM term, T& var, I min, I max)
{
  return detail::integer_nif_traits<T>::get_value(env, term, &var) && var >= T(min) &&
         var <= T(max);
}
...
inline typename std::enable_if<detail::is_nif_integer<T>::value &&
                                   detail::is_nif_integer<I>::value && std::is_unsigned<T>::value,
                               bool>::type
...
template <typename T>
inline typename std::enable_if<detail::is_nif_integer<T>::value, TERM>::type make(ErlNifEnv* env,
                                                                                  const T var)
{
...
inline typename std::enable_if<detail::container_traits<Container>::is_sequential &&
                                   !detail::container_traits<Container>::is_fixed_size,
                               bool>::type
get(ErlNifEnv* env, ERL_NIF_TERM term, Container& var)
{
  var.clear();
...
template <typename Container>
inline typename std::enable_if<detail::container_traits<Container>::is_set_like, bool>::type
get(ErlNifEnv* env, ERL_NIF_TERM term, Container& var)
{
  var.clear();
  using value_type = typename detail::container_traits<Container>::value_type;
...
template <typename Container>
inline typename std::enable_if<detail::is_consolidatable_container<Container>::value &&
                                   detail::has_reverse_iterators<Container>::value,
                               TERM>::type
...
template <typename T>
using ResourceStopEvent =
    std::function<void(T*, ErlNifEnv*, ErlNifEvent event, int is_direct_call)>;
template <typename T>
...
private:
  ...
  template <typename U, typename... Args>
  friend resource_ptr<U> construct_resource_with_events(resource_events<U> const&, Args&&... args);
  template <typename U>
...
template <class T>
void swap(resource_ptr<T>& lhs, resource_ptr<T>& rhs)
{
  lhs.swap(rhs);
}
...
template <class T>
T* get_pointer(resource_ptr<T> const& p)
{
  return p.get();
}
...
template <typename T>
ErlNifResourceType* resource_data<T>::type = 0;
/*
...
template <typename T>
inline int select_read(ErlNifEnv* env, ErlNifEvent event, T* obj, const ErlNifPid* pid, TERM msg,
                       msg_env& env_to_consume)
{
  return enif_select_read(env, event, (void*)obj, pid, msg, env_to_consume.release());
}

template <typename T>
inline int select_write(ErlNifEnv* env, ErlNifEvent event, T* obj, const ErlNifPid* pid, TERM msg,
                        msg_env& env_to_consume)
{
...
inline bool is_atom(ErlNifEnv* env, TERM term)
{
  return enif_is_atom(env, term) != 0;
}

// Check if term is a binary
inline bool is_binary(ErlNifEnv* env, TERM term)
{
  return enif_is_binary(env, term) != 0;
}

// Check if term is a reference
inline bool is_ref(ErlNifEnv* env, TERM term)
{
  return enif_is_ref(env, term) != 0;
}

// Check if term is a function
inline bool is_fun(ErlNifEnv* env, TERM term)
{
  return enif_is_fun(env, term) != 0;
}

// Check if term is a PID
inline bool is_pid(ErlNifEnv* env, TERM term)
{
  return enif_is_pid(env, term) != 0;
}

// Check if term is a port
inline bool is_port(ErlNifEnv* env, TERM term)
{
  return enif_is_port(env, term) != 0;
}

// Check if term is an empty list
inline bool is_empty_list(ErlNifEnv* env, TERM term)
{
  return enif_is_empty_list(env, term) != 0;
}

// Check if term is a list
inline bool is_list(ErlNifEnv* env, TERM term)
{
  return enif_is_list(env, term) != 0;
}

// Check if term is a tuple
inline bool is_tuple(ErlNifEnv* env, TERM term)
{
  return enif_is_tuple(env, term) != 0;
}

// Check if term is a number (integer or float)
inline bool is_number(ErlNifEnv* env, TERM term)
{
  return enif_is_number(env, term) != 0;
}

// Check if term is a map
inline bool is_map(ErlNifEnv* env, TERM term)
{
  return enif_is_map(env, term) != 0;
}

// Check if term is an exception
inline bool is_exception(ErlNifEnv* env, TERM term)
{
  return enif_is_exception(env, term) != 0;
}

// Check if two terms are identical (same object)
inline bool is_identical(TERM lhs, TERM rhs)
{
  return enif_is_identical(lhs, rhs) != 0;
}
...
template <typename T>
bool register_resource(ErlNifEnv* env, const char* name,
                       ErlNifResourceFlags flags = ErlNifResourceFlags(ERL_NIF_RT_CREATE |
                                                                       ERL_NIF_RT_TAKEOVER),
                       ErlNifResourceFlags* tried = nullptr)
{
  ErlNifResourceTypeInit init{.dtor = &detail::resource_dtor<T>,
                              .stop = &detail::resource_stop<T>,
                              .down = &detail::resource_down<T>,
                              .members = 4,
                              .dyncall = &detail::resource_dyncall<T>};

  detail::resource_data<T>::type = enif_open_resource_type_x(env, name, &init, flags, tried);
...
struct array_to_tupler {
  ...
  {
    return array_to_tupler<I - 1>::go(env, t, --end) && get(env, *end, std::get<I - 1>(t));
  }
};
...
bool get(ErlNifEnv* env, ERL_NIF_TERM term, std::tuple<Ts...>& var)
{
  ...
  return detail::array_to_tupler<std::tuple_size<std::tuple<Ts...>>::value>::go(env, var,
                                                                                array + arity);
}
/*
template<typename ...Ts>
...
typename std::enable_if_t<I == sizeof...(Ts), void> to_array(ErlNifEnv*, ERL_NIF_TERM*,
                                                             const std::tuple<Ts...>&)
{
...
template <size_t I = 0, typename... Ts>
typename std::enable_if_t<(I < sizeof...(Ts)), void> to_array(ErlNifEnv* env, ERL_NIF_TERM* a,
...
{
  if (!enif_is_list(env, term)) [[unlikely]]
    return false;
  ERL_NIF_TERM head, tail = term;
  while (enif_get_list_cell(env, tail, &head, &tail)) {
    T var;
    if (!get(env, head, var)) [[unlikely]]
...
bool get(ErlNifEnv* env, ERL_NIF_TERM term, std::array<T, N>& var)
{
  ...
  if (size_t(len) != var.size()) [[unlikely]]
    return false;
...
{
  return enif_make_map_put(env, map, make(env, var.first), make(env, var.second),
                           (ERL_NIF_TERM*)&map);
}
...
// Exceptions
//------------------------------------------------------------------------------
inline TERM badarg(ErlNifEnv* env)
{
  return TERM(enif_make_badarg(env));
}
...
inline TERM raise_exception(ErlNifEnv* env, T&& arg, Args&&... args)
{
  return TERM(enif_raise_exception(
      env, make(env, std::make_tuple(std::forward<T&&>(arg), std::forward<Args&&>(args)...))));
}
